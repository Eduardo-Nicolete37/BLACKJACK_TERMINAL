...
* **Gerenciador de Saves (Inicialização)**: Bloco `try/except` robusto capaz de ler as informações do arquivo `game_data.json` ou criar um novo arquivo contendo 3 slots de salvamento zerados se o arquivo não existir. Arquivo aberto com `encoding="utf-8"` para suporte correto a caracteres acentuados.
